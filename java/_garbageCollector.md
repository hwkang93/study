...
CMS GC 의 단점을 보완하기 위해 만들어졌으며, Java7 부터 지원을 시작하였다.

기존의 GC (위에서 언급했던 Serial, Parallel 등) 알고리즘에서는 Heap 영역을 물리적으로 Young 영역(Eden 2 + Survivor 1)과 Old 영역으로 나누어 사용하였다.
G1 GC 는 Eden 영역에 할당하고, Survivor 영역으로 카피하는 등의 과정은 동일하게 사용하지만, 물리적으로 메모리 공간을 구분하지 않는다.
대신 Region(지역) 이라는 개념을 새로 도입하여 Heap 영역을 균등하게 여러 개의 지역으로 나누고, 각 지역을 역할과 함께 논리적으로 구분하여 객체를 할당한다.
(Eden 지역인지, Survivor 지역인지, Old 지역인지)

G1 GC 에서는 Eden, Survivor, Old 역할과 Humongous 와 Available/Unused 라는 2가지 역할을 추가하였다.
Humongous 는 Region 크기의 50% 를 초과하는 객체를 저장하는 지역을 의미하며, Available/Unused 는 사용되지 않은 지역을 의미한다.

G1 GC 의 핵심은 **Heap 을 동일한 크기의 지역(Region) 으로 나누고 가비지가 많은 지역에 대해 우선적으로 GC 를 수행하는 것이다.**
그리고 G1 GC 도 다른 GC 와 마찬가지로 Minor GC, Major GC 로 나누어 수행된다.
...
그리고 이러한 작업은 Concurrent 하게 수행되기 때문에 애플리케이션의 지연도 최소화할 수 있는 것이다.


G1 GC 는 다른 GC 방식에 비해 잦게 호출된다. 하지만 작은 규모의 메모리 정리 작업이도 Concurrent 하게 수행되기 때문에 지연 속도가 적은 편이고,
가비지가 많은 지역에 대해 정리를 하므로 효율적인 GC 방법이다.

이러한 구조희 G1 GC 는 가장 처리 속도가 빠르며, 큰 메모리 공간에서 멀티 프로세스를 기반으로 운영하는 어플리케이션을 위해 고안되었다.
또한 Java9 부터 기본 가비지 컬렉터로 사용되고 있다.
