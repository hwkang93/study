...
Optional 은 **null 에 대해 안전한 코드를 작성하기 위해** 탄생했다.

Java 의 모든 객체는 null 을 가질 수 있고 null 을 가진 객체를 참조하면 NullPointerException 예외가 발생한다.
그렇기 때문에 개발자는 null 을 가질 수 있는 모든 객체에 대해 null 체크를 해야 한다.
그리고 이는 곧 에러를 만들기 좋은 코드가 된다.
